...
#define IMGUI_DISABLE_TTY_FUNCTIONS // Can't use stdout, fflush if we are not using default file
                                    // functions
#endif
IMGUI_API void* ImFileLoadToMemory(
    const char* filename, const char* mode, size_t* out_file_size = NULL, int padding_bytes = 0);
...
#define ImFloorStd(X)                                                                              \
    floorf(X) // We already uses our own ImFloor() { return (float)(int)v } internally so the
...
IMGUI_API bool
ImTriangleContainsPoint(const ImVec2& a, const ImVec2& b, const ImVec2& c, const ImVec2& p);
IMGUI_API ImVec2
ImTriangleClosestPoint(const ImVec2& a, const ImVec2& b, const ImVec2& c, const ImVec2& p);
...
// We store the chunk size first, and align the final size on 4 bytes boundaries (this what the '(X
// + 3) & ~3' statement is for) The tedious/zealous amount of casting is to avoid -Wcast-align
// warnings.
...
// ImDrawList: You may set this to higher values (e.g. 2 or 3) to increase tessellation of fast
...
struct IMGUI_API ImDrawListSharedData
{
    ...
    ImVec2
        ArcFastVtx[12 * IM_DRAWLIST_ARCFAST_TESSELLATION_MULTIPLIER]; // FIXME: Bake rounded corners
                                                                      // fill/borders in atlas
    ImU8 CircleSegmentCounts[64]; // Precomputed segment count for given radius (array index + 1)
                                  // before we calculate it dynamically (to avoid calculation
                                  // overhead)
...
struct ImGuiSettingsHandler
{
    ...
    void (*ReadLineFn)(
        ImGuiContext* ctx, ImGuiSettingsHandler* handler, void* entry,
...
struct ImGuiContext
{
    ...
    ImGuiID TestEngineHookIdInfo; // Will call test engine hooks: ImGuiTestEngineHook_IdInfo() from
                                  // GetID()
    ...
    ImVector<ImGuiWindow*>
        WindowsFocusOrder; // Windows, sorted in focus order, back to front. (FIXME: We could only
    ...
    ImGuiWindow* HoveredRootWindow; // == HoveredWindow ? HoveredWindow->RootWindow : NULL, merely a
                                    // shortcut to avoid null test in some situation.
    ...
    ImU64 ActiveIdUsingKeyInputMask; // Active widget will want to read those key inputs. When we
                                     // grow the ImGuiKey enum we'll need to either to order the
    ...
    ImVector<ImGuiID> FocusScopeStack; // Stack for PushFocusScope()/PopFocusScope() - not inherited
                                       // by Begin(), unless child window
    ...
    ImGuiNavMoveResult NavMoveResultLocalVisibleSet; // Best move request candidate within NavWindow
    ...
    int FocusRequestCurrCounterRegular;  // Any item being requested for focus, stored as an index
                                        // (we on layout to be stable between the frame pressing TAB
                                        // and the next frame, semi-ouch)
...
struct IMGUI_API ImGuiWindowTempData
{
    ...
    ImVec2 CursorMaxPos;   // Used to implicitly calculate the size of our contents, always growing
                         // during the frame. Used to calculate window->ContentSize at the beginning
                         // of next frame
    ImVec2 CurrLineSize;
    ImVec2 PrevLineSize;
    ...
    ImGuiItemStatusFlags
        LastItemStatusFlags;    // Status flags for last item (see ImGuiItemStatusFlags_)
    ImRect LastItemRect;        // Interaction rect for last item
    ImRect LastItemDisplayRect; // End-user display rect for last item (only valid if
                                // LastItemStatusFlags & ImGuiItemStatusFlags_HasDisplayRect)
...
struct IMGUI_API ImGuiWindow
{
    ...
    short BeginOrderWithinParent; // Order within immediate parent window, if we are a child window.
                                  // Otherwise 0.
...
struct ImGuiTableColumn
{
    ...
    ImGuiTableColumnFlags
        FlagsIn;                 // Flags as they were provided by user. See ImGuiTableColumnFlags_
...
// FIXME-TABLES: transient data could be stored in a per-stacked table structure: DrawSplitter,
...
struct ImGuiTable
{
    ...
    ImRect OuterRect; // Note: OuterRect.Max.y is often FLT_MAX until EndTable(), unless a height
                      // has been specified in BeginTable().
    ImRect WorkRect;
    ...
    ImDrawListSplitter
        DrawSplitter; // We carry our own ImDrawList splitter to allow recursion (FIXME: could be
                      // stored outside, worst case we need 1 splitter per recursing table)
    ImGuiTableColumnSortSpecs SortSpecsSingle;
    ImVector<ImGuiTableColumnSortSpecs>
        SortSpecsMulti; // FIXME-OPT: Using a small-vector pattern would work be good.
    ImGuiTableSortSpecs
    ...
    ImGuiTableColumnIdx
        AutoFitSingleStretchColumn; // Index of single stretch column requesting auto-fit.
    ImGuiTableColumnIdx
    ...
    ImGuiTableColumnIdx
        LeftMostStretchedColumnDisplayOrder; // Display order of left-most stretched column.
    ...
    ImGuiTableDrawChannelIdx
        Bg1DrawChannelCurrent; // For Selectable() and other widgets drawing accross columns after
...
IMGUI_API void
FocusTopMostWindowUnderOne(ImGuiWindow* under_this_window, ImGuiWindow* ignore_window);
...
IMGUI_API bool
FocusableItemRegister(ImGuiWindow* window, ImGuiID id); // Return true if focus is requested
...
// Logging/Capture
IMGUI_API void
LogBegin(ImGuiLogType type, int auto_open_depth); // -> BeginCapture() when we design v2 api, for
...
IMGUI_API ImVec2 GetNavInputAmount2d(
    ImGuiNavDirSourceFlags dir_sources, ImGuiInputReadMode mode, float slow_factor = 0.0f,
...
IMGUI_API void
SetNavIDWithRectRel(ImGuiID id, int nav_layer, ImGuiID focus_scope_id, const ImRect& rect_rel);

// Focus Scope (WIP)
// This is generally used to identify a selection set (multiple of which may be in the same window),
// as selection patterns generally need to react (e.g. clear selection) when landing on an item of
// the set.
...
IMGUI_API void TableSetColumnSortDirection(
...
TableGetHoveredColumn(); // May use (TableGetColumnFlags() & ImGuiTableColumnFlags_IsHovered)
                         ...
                         // return columns_count if the unused space at the right of visible columns
                         // is hovered.
...
IMGUI_API bool BeginTableEx(
    const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags = 0,
...
IMGUI_API ImGuiID
TableGetColumnResizeID(const ImGuiTable* table, int column_n, int instance_no = 0);
...
IMGUI_API bool
TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags);
...
IMGUI_API void RenderText(
    ImVec2 pos, const char* text, const char* text_end = NULL, bool hide_text_after_hash = true);
IMGUI_API void
...
IMGUI_API void
RenderFrame(ImVec2 p_min, ImVec2 p_max, ImU32 fill_col, bool border = true, float rounding = 0.0f);
IMGUI_API void RenderFrameBorder(ImVec2 p_min, ImVec2 p_max, float rounding = 0.0f);
IMGUI_API void RenderColorRectWithAlphaCheckerboard(
    ImDrawList* draw_list, ImVec2 p_min, ImVec2 p_max, ImU32 fill_col, float grid_step,
...
IMGUI_API void
LogRenderedText(const ImVec2* ref_pos, const char* text, const char* text_end = NULL);
...
IMGUI_API void RenderArrowPointingAt(
...
IMGUI_API bool ScrollbarEx(
    const ImRect& bb, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float avail_v,
...
IMGUI_API bool ButtonBehavior(
    const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags = 0);
...
IMGUI_API bool TreeNodeBehavior(
    ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end = NULL);
...
IMGUI_API float ScaleRatioFromValueT(
    ImGuiDataType data_type, T v, T v_min, T v_max, bool is_logarithmic,
    float logarithmic_zero_epsilon, float zero_deadzone_size);
template <typename T, typename SIGNED_T, typename FLOAT_T>
IMGUI_API T ScaleValueFromRatioT(
    ImGuiDataType data_type, float t, T v_min, T v_max, bool is_logarithmic,
    float logarithmic_zero_epsilon, float zero_deadzone_size);
template <typename T, typename SIGNED_T, typename FLOAT_T>
IMGUI_API bool DragBehaviorT(
    ImGuiDataType data_type, T* v, float v_speed, T v_min, T v_max, const char* format,
    ImGuiSliderFlags flags);
template <typename T, typename SIGNED_T, typename FLOAT_T>
IMGUI_API bool SliderBehaviorT(
    const ImRect& bb, ImGuiID id, ImGuiDataType data_type, T* v, T v_min, T v_max,
...
IMGUI_API void DataTypeApplyOp(
    ImGuiDataType data_type, int op, void* output, const void* arg_1, const void* arg_2);
IMGUI_API bool DataTypeApplyOpFromText(
...
// Debug Tools
IMGUI_API void
ErrorCheckEndFrameRecover(ImGuiErrorLogCallback log_callback, void* user_data = NULL);
...
IMGUI_API void
DebugNodeDrawList(ImGuiWindow* window, const ImDrawList* draw_list, const char* label);
...
IMGUI_API void ImFontAtlasBuildRender1bppRectFromString(
    ImFontAtlas* atlas, int atlas_x, int atlas_y, int w, int h, const char* in_str,
    char in_marker_char, unsigned char in_marker_pixel_value);
IMGUI_API void
ImFontAtlasBuildMultiplyCalcLookupTable(unsigned char out_table[256], float in_multiply_factor);
...
extern void ImGuiTestEngineHook_ItemInfo(
    ImGuiContext* ctx, ImGuiID id, const char* label, ImGuiItemStatusFlags flags);
extern void ImGuiTestEngineHook_IdInfo(
    ImGuiContext* ctx, ImGuiDataType data_type, ImGuiID id, const void* data_id);
extern void ImGuiTestEngineHook_IdInfo(
    ImGuiContext* ctx, ImGuiDataType data_type, ImGuiID id, const void* data_id,
