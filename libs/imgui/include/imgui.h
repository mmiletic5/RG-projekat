...
// FIXME-TABLE: Add ImGuiTableSortSpecs and ImGuiTableColumnSortSpecs in "Misc data structures"
section above (we don't do it right now to facilitate merging various branches)

*/

#pragma once

// Configuration file with compile-time options (edit imconfig.h or #define IMGUI_USER_CONFIG to
// your own filename)
...
#define IM_ARRAYSIZE(_ARR)                                                                         \
    ((int)(sizeof(_ARR) / sizeof(*(_ARR)))) // Size of a static C-style array. Don't use on
                                            // pointers!
...
#define IM_FMTARGS(FMT)                                                                            \
    __attribute__((                                                                                \
        format(printf, FMT, FMT + 1))) // Apply printf-style warnings to our formatting functions.
#define IM_FMTLIST(FMT) __attribute__((format(printf, FMT, 0)))
#elif !defined(IMGUI_USE_STB_SPRINTF) && defined(__GNUC__) && defined(__MINGW32__)
#define IM_FMTARGS(FMT)                                                                            \
    __attribute__((format(                                                                         \
        gnu_printf, FMT, FMT + 1))) // Apply printf-style warnings to our formatting functions.
...
typedef int ImGuiTableBgTarget; // -> enum ImGuiTableBgTarget_   // Enum: A color target for
                                // TableSetBgColor()
typedef int ImDrawCornerFlags; // -> enum ImDrawCornerFlags_    // Flags: for ImDrawList::AddRect(),
                               // AddRectFilled() etc.
...
typedef int ImGuiInputTextFlags;  // -> enum ImGuiInputTextFlags_  // Flags: for InputText(),
                                  // InputTextMultiline()
typedef int ImGuiKeyModFlags;     // -> enum ImGuiKeyModFlags_     // Flags: for io.KeyMods
...
// Other types
#ifndef ImTextureID // ImTextureID [configurable type: override in imconfig.h with '#define
                    // ImTextureID xxx']
typedef void*
    ImTextureID; // User data for rendering backend to identify a texture. This is whatever to you
                 // want it to be! read the FAQ about ImTextureID for details.
#endif
typedef unsigned int
    ImGuiID; // A unique ID used by widgets, typically hashed from a stack of string.
...
// (This is a namespace. You can add extra ImGui:: functions in your own separate file. Please don't
...
// CreateContext() to share a font atlas between imgui contexts. None of those functions is reliant
...
EndFrame(); // ends the Dear ImGui frame. automatically called by Render(). If you don't need to
            ...
            // you'll have wasted CPU already! If you don't need to render, better to not create any
...
IMGUI_API void ShowDemoWindow(
    bool* p_open = NULL); // create Demo window (previously called ShowTestWindow). demonstrate most
                          // ImGui features. call this to learn about the library! try to make it
                          // always available in your application!
IMGUI_API void
ShowAboutWindow(bool* p_open = NULL); // create About window. display Dear ImGui version, credits
...
IMGUI_API void ShowFontSelector(const char* label); // add font selector block (not a window),
                                                    // essentially a combo listing the loaded fonts.
IMGUI_API void ShowUserGuide(); // add basic help/info block (not a window): how to manipulate ImGui
...
IMGUI_API void SetNextWindowContentSize(
    const ImVec2&
        size); // set next window content size (~ scrollable client area, which enforce the range of
...
IMGUI_API void
SetWindowFontScale(float scale); // set font scale. Adjust IO.FontGlobalScale if you want to scale
                                 // all windows. This is an old API! For correct scaling, prefer to
...
IMGUI_API ImVec2
GetWindowContentRegionMax(); // content boundaries max (roughly (0,0)+Size-Scroll) where Size can be
...
IMGUI_API void
PushButtonRepeat(bool repeat); // in 'repeat' mode, Button*() functions return repeated true in a
...
IMGUI_API ImU32
GetColorU32(ImGuiCol idx, float alpha_mul = 1.0f); // retrieve given style color with style alpha
...
IMGUI_API void PushItemWidth(
    float item_width); // push width of items for common large "item+label" widgets. >0.0f: width in
...
IMGUI_API void
SetNextItemWidth(float item_width); // set width of the _next_ common large "item+label" widget.
                                    // >0.0f: width in pixels, <0.0f align xx pixels to the right of
                                    // window (so -FLT_MIN always align width to the right side)
IMGUI_API float
CalcItemWidth(); // width of item given pushed settings and current cursor position. NOT necessarily
...
IMGUI_API void EndGroup(); // unlock horizontal starting position + capture the whole group bounding
                           // box into one "item" (so you can use IsItemHovered() or layout
...
IMGUI_API void TextWrapped(const char* fmt, ...)
    IM_FMTARGS(1); // shortcut for PushTextWrapPos(0.0f); Text(fmt, ...); PopTextWrapPos();. Note
                   // that this won't work on an auto-resizing window if there's no other widgets to
...
IMGUI_API void
ProgressBar(float fraction, const ImVec2& size_arg = ImVec2(-1, 0), const char* overlay = NULL);
IMGUI_API void
...
IMGUI_API bool DragFloat2(
    const char* label, float v[2], float v_speed = 1.0f, float v_min = 0.0f, float v_max = 0.0f,
    const char* format = "%.3f", ImGuiSliderFlags flags = 0);
IMGUI_API bool DragFloat3(
    const char* label, float v[3], float v_speed = 1.0f, float v_min = 0.0f, float v_max = 0.0f,
    const char* format = "%.3f", ImGuiSliderFlags flags = 0);
IMGUI_API bool DragFloat4(
    const char* label, float v[4], float v_speed = 1.0f, float v_min = 0.0f, float v_max = 0.0f,
    const char* format = "%.3f", ImGuiSliderFlags flags = 0);
IMGUI_API bool DragFloatRange2(
    const char* label, float* v_current_min, float* v_current_max, float v_speed = 1.0f,
...
IMGUI_API bool DragInt2(
    const char* label, int v[2], float v_speed = 1.0f, int v_min = 0, int v_max = 0,
    const char* format = "%d", ImGuiSliderFlags flags = 0);
IMGUI_API bool DragInt3(
    const char* label, int v[3], float v_speed = 1.0f, int v_min = 0, int v_max = 0,
    const char* format = "%d", ImGuiSliderFlags flags = 0);
IMGUI_API bool DragInt4(
    const char* label, int v[4], float v_speed = 1.0f, int v_min = 0, int v_max = 0,
    const char* format = "%d", ImGuiSliderFlags flags = 0);
IMGUI_API bool DragIntRange2(
    const char* label, int* v_current_min, int* v_current_max, float v_speed = 1.0f, int v_min = 0,
    int v_max = 0, const char* format = "%d", const char* format_max = NULL,
...
IMGUI_API bool SliderFloat2(
    const char* label, float v[2], float v_min, float v_max, const char* format = "%.3f",
    ImGuiSliderFlags flags = 0);
IMGUI_API bool SliderFloat3(
    const char* label, float v[3], float v_min, float v_max, const char* format = "%.3f",
    ImGuiSliderFlags flags = 0);
IMGUI_API bool SliderFloat4(
    const char* label, float v[4], float v_min, float v_max, const char* format = "%.3f",
    ImGuiSliderFlags flags = 0);
IMGUI_API bool SliderAngle(
    const char* label, float* v_rad, float v_degrees_min = -360.0f, float v_degrees_max = +360.0f,
...
IMGUI_API bool SliderInt2(
    const char* label, int v[2], int v_min, int v_max, const char* format = "%d",
    ImGuiSliderFlags flags = 0);
IMGUI_API bool SliderInt3(
    const char* label, int v[3], int v_min, int v_max, const char* format = "%d",
    ImGuiSliderFlags flags = 0);
IMGUI_API bool SliderInt4(
    const char* label, int v[4], int v_min, int v_max, const char* format = "%d",
    ImGuiSliderFlags flags = 0);
...
IMGUI_API bool SliderScalarN(
    const char* label, ImGuiDataType data_type, void* p_data, int components, const void* p_min,
...
IMGUI_API bool VSliderInt(
    const char* label, const ImVec2& size, int* v, int v_min, int v_max, const char* format = "%d",
    ImGuiSliderFlags flags = 0);
...
IMGUI_API bool InputText(
    const char* label, char* buf, size_t buf_size, ImGuiInputTextFlags flags = 0,
    ImGuiInputTextCallback callback = NULL, void* user_data = NULL);
IMGUI_API bool InputTextMultiline(
    const char* label, char* buf, size_t buf_size, const ImVec2& size = ImVec2(0, 0),
    ImGuiInputTextFlags flags = 0, ImGuiInputTextCallback callback = NULL, void* user_data = NULL);
IMGUI_API bool InputTextWithHint(
    const char* label, const char* hint, char* buf, size_t buf_size, ImGuiInputTextFlags flags = 0,
    ImGuiInputTextCallback callback = NULL, void* user_data = NULL);
IMGUI_API bool InputFloat(
    const char* label, float* v, float step = 0.0f, float step_fast = 0.0f,
...
IMGUI_API bool InputDouble(
    const char* label, double* v, double step = 0.0, double step_fast = 0.0,
    const char* format = "%.6f", ImGuiInputTextFlags flags = 0);
IMGUI_API bool InputScalar(
    const char* label, ImGuiDataType data_type, void* p_data, const void* p_step = NULL,
...
IMGUI_API bool TreeNode(const char* str_id, const char* fmt, ...)
    IM_FMTARGS(2); // helper variation to easily decorelate the id from the displayed string. Read
                   // the FAQ about why and how to use ID. to align arbitrary text at the same level
...
TreeNodeExV(const void* ptr_id, ImGuiTreeNodeFlags flags, const char* fmt, va_list args)
    IM_FMTLIST(3);
IMGUI_API void
TreePush(const char* str_id); // ~ Indent()+PushId(). Already called by TreeNode() when returning
...
//  - BeginPopupModal(): block every interactions behind the window, cannot be closed by user, add a
...
IMGUI_API void OpenPopupOnItemClick(
    ...
    ImGuiPopupFlags popup_flags = 1); // helper to open popup when clicked on last item. return true
                                      // when just opened. (note: actually triggers on the mouse
...
//  - IMPORTANT: we exceptionally default their flags to 1 (== ImGuiPopupFlags_MouseButtonRight) for
//  backward compatibility with older API taking 'int mouse_button = 1' parameter, so if you add
//  other flags remember to re-add the ImGuiPopupFlags_MouseButtonRight.
IMGUI_API bool BeginPopupContextItem(
    const char* str_id = NULL,
    ImGuiPopupFlags popup_flags =
        1); // open+begin popup when clicked on last item. if you can pass a NULL str_id only if the
            // previous item had an id. If you want to use that on a non-interactive item such as
...
// - 4. Optionally call TableHeadersRow() to submit a header row (names will be pulled from data
// submitted to TableSetupColumns)
// - 5. Populate contents
...
//    performing
//      width measurements. Otherwise, you may skip submitting the contents of a cell/column, BUT
//      ONLY if you know it is not going to contribute to row height. In many situations, you may
//      skip submitting contents for every columns but one (e.g. the first one).
...
IMGUI_API bool BeginTable(
    const char* str_id, int columns_count, ImGuiTableFlags flags = 0,
...
// other flags etc.
//   Important: this will not display anything! The name passed to TableSetupColumn() is used by
//   TableHeadersRow() and context-menus.
// - Use TableHeadersRow() to create a row and automatically submit a TableHeader() for each column.
//   Headers are required to perform: reordering, sorting, and opening the context menu (but context
//   menu can also be available in columns body using ImGuiTableFlags_ContextMenuInBody).
// - You may manually submit headers using TableNextRow() + TableHeader() calls, but this is only
// useful in some advanced cases (e.g. adding custom widgets in header row).
// - Use TableSetupScrollFreeze() to lock columns (from the right) or rows (from the top) so they
// stay visible when scrolled.
IMGUI_API void TableSetupColumn(
    const char* label, ImGuiTableColumnFlags flags = 0, float init_width_or_weight = -1.0f,
    ImU32 user_id = 0);
IMGUI_API void
...
// - Most functions taking 'int column_n' treat the default value of -1 as the same as passing the
// current column index
// - Sorting: call TableGetSortSpecs() to retrieve latest sort specs for the table. Return value
// will be NULL if no sorting.
//   When 'SpecsDirty == true' you should sort your data. It will be true when sorting specs have
//   changed since last call, or the first time. Make sure to set 'SpecsDirty = false' after
//   sorting, else you may wastefully sort your data every frame! Lifetime: don't hold on this
...
TableGetColumnFlags(int column_n = -1); // return column flags so you can query their
...
IMGUI_API void TableSetBgColor(
    ...
    int column_n = -1); // change the color of a cell, row, or column. See ImGuiTableBgTarget_ flags
                        // for details.
...
IMGUI_API void SetColumnOffset(
    int column_index, float offset_x); // set position of column line (in pixels, from the left side
...
IMGUI_API void SetTabItemClosed(
    const char*
        tab_or_docked_window_label); // notify TabBar or Docking system of a closed tab/window ahead
                                     // (useful to reduce visual flicker on reorderable tab bars).
                                     // For tab-bar: call after BeginTabBar() and before Tab
...
IMGUI_API void
LogToFile(int auto_open_depth = -1, const char* filename = NULL); // start logging to file
...
// preview tooltip (we currently display a fallback "..." tooltip as replacement)
IMGUI_API bool BeginDragDropSource(
    ImGuiDragDropFlags flags = 0); // call when the current item is active. If this return true, you
                                   // can call SetDragDropPayload() + EndDragDropSource()
IMGUI_API bool SetDragDropPayload(
    const char* type, const void* data, size_t sz,
    ImGuiCond cond =
        0); // type is a user defined string of maximum 32 characters. Strings starting with '_' are
...
IMGUI_API bool IsItemClicked(
    ImGuiMouseButton mouse_button = 0); // is the last item clicked? (e.g. button/node just clicked
                                        // on) == IsMouseClicked(mouse_button) && IsItemHovered()
IMGUI_API bool IsItemVisible(); // is the last item visible? (items may be out of sight because of
                                // clipping/scrolling)
IMGUI_API bool
IsItemEdited(); // did the last item modify its underlying value this frame? or was pressed? This is
...
IMGUI_API void CalcListClipping(
    ...
    int*
        out_items_display_end); // calculate coarse clipping for large list of evenly sized items.
...
// - We don't know the meaning of those value. You can use GetKeyIndex() to map a ImGuiKey_ value
// into the user index.
IMGUI_API int
GetKeyIndex(ImGuiKey imgui_key); // map ImGuiKey_* values into user's key index. == io.KeyMap[key]
IMGUI_API bool IsKeyDown(int user_key_index); // is key being held. == io.KeysDown[user_key_index].
IMGUI_API bool IsKeyPressed(
    int user_key_index, bool repeat = true); // was key pressed (went from !Down to Down)? if
...
IMGUI_API bool IsMouseDoubleClicked(
    ImGuiMouseButton button); // did mouse button double-clicked? (note that a double-click will
...
GetMouseCursor(); // get desired cursor type, reset in ImGui::NewFrame(), this is updated during the
...
IMGUI_API void CaptureMouseFromApp(
    bool want_capture_mouse_value =
        true); // attention: misleading name! manually override io.WantCaptureMouse flag next frame
               // (said flag is entirely left for your application to handle). This is equivalent to
               // setting "io.WantCaptureMouse = want_capture_mouse_value;" after the next
               // NewFrame() call.
...
IMGUI_API void SaveIniSettingsToDisk(
    const char* ini_filename); // this is automatically called (if io.IniFilename is not empty) a
                               // few seconds after any modification that should be reflected in the
...
IMGUI_API const char* SaveIniSettingsToMemory(
    size_t* out_ini_size =
        NULL); // return a zero-terminated string with the .ini data which you can save by your own
               // mean. call when io.WantSaveIniSettings is set, then save data by your own mean and
               // clear io.WantSaveIniSettings.
...
//   on the default parameter and want to another another flag, you need to pass in the
...
// - Important! Sizing policies have particularly complex and subtle side effects, more so than you
// would expect.
//   Read comments/demos carefully + experiment with live demos to get acquainted with them.
// - The default sizing policy for columns depends on whether the ScrollX flag is set on the table:
//   When ScrollX is off:
//    - Table defaults to ImGuiTableFlags_ColumnsWidthStretch -> all Columns defaults to
...
//    all.
//   When ScrollX is on:
//    - Table defaults to ImGuiTableFlags_ColumnsWidthFixed -> all Columns defaults to
...
// - Mixing up columns with different sizing policy is possible BUT can be tricky and has some
// side-effects and restrictions.
//   (their visible order and the scrolling state have subtle but necessary effects on how they can
//   be manually resized). The typical use of mixing sizing policies is to have ScrollX disabled,
//   one or two Stretch Column and many Fixed Columns.
enum ImGuiTableFlags_
{
    ...
    ImGuiTableFlags_Reorderable =
        1 << 1, // Allow reordering columns in header row (need calling TableSetupColumn() +
                // TableHeadersRow() to display headers)
    ImGuiTableFlags_Hideable = 1 << 2, // Allow hiding/disabling columns in context menu.
    ImGuiTableFlags_Sortable = 1 << 3, // Allow sorting on one column (sort_specs_count will always
                                       // be == 1). Call TableGetSortSpecs() to obtain sort specs.
    ImGuiTableFlags_MultiSortable =
        1 << 4, // Allow sorting on multiple columns by holding Shift (sort_specs_count may be > 1).
                // Call TableGetSortSpecs() to obtain sort specs.
    ImGuiTableFlags_NoSavedSettings =
        1 << 5, // Disable persisting columns order, width and sort settings in the .ini file.
    ImGuiTableFlags_ContextMenuInBody =
        1 << 6, // Right-click on columns body/contents will display table context menu. By default
                // it is available in TableHeadersRow(). Decorations
    ImGuiTableFlags_RowBg =
        1
        << 7, // Set each RowBg color with ImGuiCol_TableRowBg or ImGuiCol_TableRowBgAlt (equivalent
    ...
    ImGuiTableFlags_NoBordersInBody =
        1
        << 12, // Disable vertical borders in columns Body (borders will always appears in Headers).
    ImGuiTableFlags_NoBordersInBodyUntilResize =
        1 << 13, // Disable vertical borders in columns Body until hovered for resize (borders will
                 // always appears in Headers). Sizing
    ImGuiTableFlags_ColumnsWidthStretch =
        1 << 14, // Default if ScrollX is off. Columns will default to use _WidthStretch. Read
                 // description above for more details.
    ImGuiTableFlags_ColumnsWidthFixed =
        1 << 15, // Default if ScrollX is on. Columns will default to use _WidthFixed or
                 // _WidthAutoResize policy (if Resizable is off). Read description above for more
                 // details.
    ImGuiTableFlags_SameWidths =
        1 << 16, // Make all columns the same widths which is useful with Fixed columns policy (but
                 // granted by default with Stretch policy + no resize). Implicitly enable
                 // ImGuiTableFlags_NoKeepColumnsVisible and disable ImGuiTableFlags_Resizable.
    ImGuiTableFlags_NoHeadersWidth =
        1 << 17, // Disable headers' contribution to automatic width calculation.
    ImGuiTableFlags_NoHostExtendY =
        1
        << 18, // Disable extending past the limit set by outer_size.y, only meaningful when neither
               // of ScrollX|ScrollY are set (data below the limit will be clipped and not visible)
    ImGuiTableFlags_NoKeepColumnsVisible = 1
                                           << 19, // Disable keeping column always minimally visible
                                                  // when ScrollX is off and table gets too small.
    ImGuiTableFlags_PreciseWidths =
        1
        << 20, // Disable distributing remainder width to stretched columns (width allocation on a
    ...
    ImGuiTableFlags_NoClip =
        1 << 21, // Disable clipping rectangle for every individual columns (reduce draw command
                 // count, items will be able to overflow into other columns). Generally
                 // incompatible with TableSetupScrollFreeze(). Padding
    ImGuiTableFlags_PadOuterX =
        1 << 22, // Default if BordersOuterV is on. Enable outer-most padding.
    ImGuiTableFlags_NoPadOuterX =
        1 << 23, // Default if BordersOuterV is off. Disable outer-most padding.
    ImGuiTableFlags_NoPadInnerX =
        1 << 24, // Disable inner padding between columns (double inner padding if BordersOuterV is
                 // on, single inner padding if BordersOuterV is off). Scrolling
    ImGuiTableFlags_ScrollX =
        1 << 25, // Enable horizontal scrolling. Require 'outer_size' parameter of BeginTable() to
    ...
    ImGuiTableFlags_ScrollY = 1 << 26 // Enable vertical scrolling. Require 'outer_size' parameter
...
enum ImGuiTableColumnFlags_
{
    ...
    ImGuiTableColumnFlags_WidthStretch =
        1 << 2, // Column will stretch. Preferable with horizontal scrolling disabled (default if
                // table sizing policy is _ColumnsWidthStretch).
    ImGuiTableColumnFlags_WidthFixed =
        1 << 3, // Column will not stretch. Preferable with horizontal scrolling enabled (default if
                // table sizing policy is _ColumnsWidthFixed and table is resizable).
    ImGuiTableColumnFlags_WidthAutoResize =
        1 << 4, // Column will not stretch and keep resizing based on submitted contents (default if
                // table sizing policy is _ColumnsWidthFixed and table is not resizable).
    ...
    ImGuiTableColumnFlags_PreferSortAscending =
        1 << 13, // Make the initial sort direction Ascending when first sorting on this column
                 // (default).
    ...
    ImGuiTableColumnFlags_IndentEnable =
        1 << 15, // Use current Indent value when entering cell (default for column 0).
    ImGuiTableColumnFlags_IndentDisable =
        1 << 16, // Ignore current Indent value when entering cell (default for columns > 0).
...
enum ImGuiTableRowFlags_
{
    ...
    ImGuiTableRowFlags_Headers = 1
                                 << 0 // Identify header row (set default background color + width
...
enum ImGuiTableBgTarget_
{
    ...
    // ImGuiTableBgTarget_ColumnBg0                  = 1,        // FIXME-TABLE: Todo. Set column
    // background color 0 (generally used for background ImGuiTableBgTarget_ColumnBg1 = 2,        //
    // FIXME-TABLE: Todo. Set column background color 1 (generally used for selection marking)
    ImGuiTableBgTarget_RowBg0 = 3, // Set row background color 0 (generally used for background,
                                   // automatically set when ImGuiTableFlags_RowBg is used)
    ImGuiTableBgTarget_RowBg1 =
        4, // Set row background color 1 (generally used for selection marking)
...
enum ImGuiFocusedFlags_
{
    ...
    ImGuiFocusedFlags_RootWindow =
        1
        << 1, // IsWindowFocused(): Test from root window (top most parent of the current hierarchy)
...
// io.ConfigFlags |= ImGuiConfigFlags_NavEnableGamepad to enable. Backend: set
...
enum ImGuiNavInput_
{
    ...
    ImGuiNavInput_FocusPrev, // next window (w/ PadMenu)                     // e.g. L1 or L2 (PS4),
                             // LB or LT (Xbox), L or ZL (Switch)
    ImGuiNavInput_FocusNext, // prev window (w/ PadMenu)                     // e.g. R1 or R2 (PS4),
                             // RB or RT (Xbox), R or ZL (Switch)
    ImGuiNavInput_TweakSlow, // slower tweaks                                // e.g. L1 or L2 (PS4),
                             // LB or LT (Xbox), L or ZL (Switch)
    ImGuiNavInput_TweakFast, // faster tweaks                                // e.g. R1 or R2 (PS4),
                             // RB or RT (Xbox), R or ZL (Switch)
...
struct ImGuiIO
{
    ...
    float MouseDragThreshold;   // = 6.0f           // Distance threshold before considering we are
                                // dragging.
    int KeyMap[ImGuiKey_COUNT]; // <unset>          // Map of indices into the KeysDown[512] entries
                                // array which represent your "native" keyboard state.
    ...
    bool ConfigInputTextCursorBlink; // = true           // Set to false to disable blinking cursor,
                                     // for users who consider it distracting. (was called:
    ...
    float ConfigMemoryCompactTimer; // = 60.0f          // [BETA] Free transient windows/tables
                                    // memory buffers when unused for given amount of time. Set to
                                    // -1.0f to disable.
    ...
    bool MouseDown[5];  // Mouse buttons: 0=left, 1=right, 2=middle + extras (ImGuiMouseButton_COUNT
                        // == 5). Dear ImGui mostly uses left and right buttons. Others buttons
                        // allows us to track if the mouse is being used by your application +
    ...
    IMGUI_API void
    AddInputCharactersUTF8(const char* str); // Queue new characters input from an UTF-8 string
    ...
    ImGuiKeyModFlags KeyMods;   // Key mods flags (same as io.KeyCtrl/KeyShift/KeyAlt/KeySuper but
...
struct ImGuiTableSortSpecs
{
    ...
    int SpecsCount;  // Sort spec count. Most often 1 unless e.g. ImGuiTableFlags_MultiSortable is
                     // enabled.
...
static inline bool SliderFloat2(
    const char* label, float v[2], float v_min, float v_max, const char* format, float power)
{
    return SliderScalarN(label, ImGuiDataType_Float, v, 2, &v_min, &v_max, format, power);
}
static inline bool SliderFloat3(
    const char* label, float v[3], float v_min, float v_max, const char* format, float power)
{
    return SliderScalarN(label, ImGuiDataType_Float, v, 3, &v_min, &v_max, format, power);
}
static inline bool SliderFloat4(
...
// the non-visible items we have skipped. (Dear ImGui already clip items based on their bounds but
// it needs to measure text size to do so, whereas manual coarse clipping before submission makes
// this cost and your own data fetching/submission cost almost null) Usage:
...
// - VtxOffset/IdxOffset: When 'io.BackendFlags & ImGuiBackendFlags_RendererHasVtxOffset' is
// enabled,
...
// ImGuiBackendFlags_RendererHasVtxOffset' and handle ImDrawCmd::VtxOffset in the renderer backend
...
// Flags for ImDrawList. Those are set automatically by ImGui:: functions from ImGuiIO settings, and
// generally not manipulated directly. It is however possible to temporarily alter flags between
...
struct ImDrawList
{
    ...
    IMGUI_API void AddRect(
        ...
        float thickness =
            1.0f); // a: upper-left, b: lower-right (== upper-left + size), rounding_corners_flags:
    ...
    IMGUI_API void AddRectFilledMultiColor(
        const ImVec2& p_min, const ImVec2& p_max, ImU32 col_upr_left, ImU32 col_upr_right,
    ...
    IMGUI_API void AddText(
        const ImFont* font, float font_size, const ImVec2& pos, ImU32 col, const char* text_begin,
    ...
    IMGUI_API void AddImage(
        ImTextureID user_texture_id, const ImVec2& p_min, const ImVec2& p_max,
        const ImVec2& uv_min = ImVec2(0, 0), const ImVec2& uv_max = ImVec2(1, 1),
        ImU32 col = IM_COL32_WHITE);
    IMGUI_API void AddImageQuad(
        ImTextureID user_texture_id, const ImVec2& p1, const ImVec2& p2, const ImVec2& p3,
    ...
    IMGUI_API void AddImageRounded(
        ImTextureID user_texture_id, const ImVec2& p_min, const ImVec2& p_max, const ImVec2& uv_min,
    ...
    IMGUI_API void
    PathArcTo(const ImVec2& center, float radius, float a_min, float a_max, int num_segments = 10);
    ...
    IMGUI_API void PrimQuadUV(
        const ImVec2& a, const ImVec2& b, const ImVec2& c, const ImVec2& d, const ImVec2& uv_a,
...
struct ImFontConfig
{
    ...
    const ImWchar* GlyphRanges; // NULL     // Pointer to a user-provided list of Unicode range (2
                                // value per range, values are inclusive, zero-terminated list). THE
                                // ARRAY DATA NEEDS TO PERSIST AS LONG AS THE FONT IS ALIVE.
...
struct ImFontAtlas
{
    ...
    // docs/FONTS.md for more details about using colorful icons. Note: this API may be redesigned
    // later in order to support multi-monitor varying DPI settings.
    ...
    IMGUI_API bool GetMouseCursorTexData(
        ImGuiMouseCursor cursor, ImVec2* out_offset, ImVec2* out_size, ImVec2 out_uv_border[2],
...
struct ImFont
{
    ...
    short ConfigDataCount;  // 2     // in  // ~ 1        // Number of ImFontConfig involved in
                            ...
                            // into one ImFont.
    ImWchar FallbackChar;   // 2     // in  // = '?'      // Replacement character if a glyph isn't
    ...
    ImU8 Used4kPagesMap
        [(IM_UNICODE_CODEPOINT_MAX + 1) / 4096 /
         8]; // 2 bytes if ImWchar=ImWchar16, 34 bytes if ImWchar==ImWchar32. Store 1-bit for each
    ...
    IMGUI_API ImVec2 CalcTextSizeA(
        float size, float max_width, float wrap_width, const char* text_begin,
    ...
    IMGUI_API void
    RenderChar(ImDrawList* draw_list, float size, ImVec2 pos, ImU32 col, ImWchar c) const;
    IMGUI_API void RenderText(
        ImDrawList* draw_list, float size, ImVec2 pos, ImU32 col, const ImVec4& clip_rect,
    ...
    IMGUI_API void AddGlyph(
        const ImFontConfig* src_cfg, ImWchar c, float x0, float y0, float x1, float y1, float u0,
