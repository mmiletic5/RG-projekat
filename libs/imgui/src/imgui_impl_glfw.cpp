...
//  with 'io.ConfigFlags |= ImGuiConfigFlags_NoMouseCursorChange' (note: the resizing cursors
//  requires GLFW 3.4+). [X] Platform: Keyboard arrays indexed using GLFW_KEY_* codes, e.g.
...
static auto
ImGui_ImplGlfw_Init(GLFWwindow* window, bool install_callbacks, GlfwClientApi client_api) -> bool
{
    g_Window = window;
    g_Time = 0.0;
...
static void ImGui_ImplGlfw_UpdateMouseCursor()
{
    ...
    else
    {
        ...
        // but 3.3 works here.
        glfwSetCursor(
            g_Window, g_MouseCursors[imgui_cursor] ? g_MouseCursors[imgui_cursor]
...
void ImGui_ImplGlfw_NewFrame()
{
    ...
    IM_ASSERT(
        io.Fonts->IsBuilt() &&
        "Font atlas not built! It is generally built by the renderer backend. Missing call to "
